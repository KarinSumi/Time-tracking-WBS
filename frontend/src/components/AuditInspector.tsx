...
const AuditInspector: React.FC<{ logs: AuditLog[] }> = ({ logs }) => {
  return (
    <div className="">
      <h2 className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-6 ml-1">Audit History</h2>
        ...
        {logs.map((log) => (
          <div key={log.id} className="relative pl-6 border-l-[0.5px] border-white/20">
            ...
            <div className="text-sm">
              <p className="text-white font-medium mb-4">Modified by <span className="text-white/80">{log.performedBy?.split(' ')[0] || 'Unknown'}</span></p>
